...
def create_mixture_yaml(args):
    ...
    out_yaml_path = os.path.join(workspace, 'yaml_files', mixture_type, 'mixture.yaml')
    ...
    for fold in folds:
        ...
        for n in range(mixed_audios_per_fold):
            ...
            for j, index in enumerate(event_indexes_for_one_clip):
                ...
                if mixture_type=='rand_init' or mixture_type=='mix':
                    onset = 0.5 + np.random.uniform(0., 6.)
                else:
                    onset = j * 2.5 + 0.5
                # Onsets of events are 0.5 s, 3.0 s,
                ...
                if mixture_type=='no_clip' or mixture_type=='mix':
                    audio_duration = min(audio_duration, 5.0)
                else:
                    audio_duration = min(audio_duration, 2.0)
                    # Clip maximum duration to 2.0 s.
                
                offset = min(onset + audio_duration, 10)
                ...
                avg_length += offset-onset
                evt_length.append(offset-onset)
                total_count+= 1
    ...
    create_folder(os.path.join(workspace, '_tmp', mixture_type))
    pickle.dump(data_list, open(os.path.join(workspace, '_tmp', mixture_type, 'mixture_yaml.p'), 'wb'))
        
    print('Write out mixture yaml to {}'.format(out_yaml_path))
    print(avg_length/total_count)
    print(np.mean(evt_length))
    print(np.std(evt_length))
...
if __name__ == '__main__':
    ...
    parser_a.add_argument('--mixture_type', type=str,
    ...
    parser_b.add_argument('--mixture_type', type=str,
                          choices=['original', 'rand_init', 'no_clip', 'mix'], required=True)
